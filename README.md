...
+ Изменить цикл по полям, добавить все поля
+ Добавить все поля в команду перевода указателей
+ Добавить перевод указателей в конец цикла и выход организовать по завершению данных
- Проверить правильность работы сообщений об ошибках данных
+ Звуковой сигнал при ошибке
+ При создании или открытии проекта остаются старые настройки источника данных, нужно сбрасывать
+ Отключение не подключенного источника данных вызывает ошибку
+ При подключении источника данных из другого места с заменой файл удаляется, но не копируется и не подключается
+ Пропала подсказка при включении записи скриншота
+ Сделать файлы скрипта более удобочитаемыми в редакторе
- 

Реализация программы выбора и Запуска скриптов
- маленькое окно с активным полем ввода, позволяющее ввести код из 2-4 цифр 
для запуска скрипта (можно через сканер штрих-кодов), ввод клавишей Enter. 
2 кнопки для перехода в редактор скриптов и окно выбора для запуска скрипта.
- окно выбора скрипта, содержит список скриптов, с датами создания/изменения, 
описанием и номером. Автоматически генерирует номер при добавлении нового проекта.  
Номер состоит из 2 частей, номера самого проекта в рабочей папке и номера файла данных в нем.
- рабочая папка содержит папки проектов и список 