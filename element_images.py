...
def pattern_search(name_template: str, x_point: int = 0, y_point: int = 0) -> tuple:
    ...
    которое ищет в папке изображений проекта. Второй и третий аргументы - координаты на экране
    где должна присутствовать кнопка. Если есть четвертый аргумент, то это количество попыток поиска.
    Он приходит от команды Проверка изображения, у которой свои настройки по количеству попыток
    и не используется поиск по всему экрану. В этом случае количество попыток для обычного клика мыши
    подменяется на эту переменную, а она сама ставится True, чтобы предотвратить поиск по всему экрану.
    Не производит поиск и сообщает о наличии, если пришло пустое имя файла или количество попыток 0.
    Повторит поиск указанное количество раз с промежутком в секунду. Если не найдет изображение в указанном месте
    вернет исключение ElementNotFound. Но для проверки перед кликом мыши может произвести поиск по всему экрану
